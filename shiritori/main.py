...
def update_score():
  ...
  for first_letter in words_beginning_with:
    ...
    words_beginning_with[first_letter].sort(key=lambda word: word.score)  # 最初の文字ごとにscoreでソート
  ...
  # 同じ文字で終わる単語が複数ある場合は、一つ以外のscoreを極端に大きくする
  # そうすることでscoreの高い順に探索する時の無駄がなくなる
  for first_letter in words_beginning_with:
    ...
    words_beginning_with[first_letter].sort(key=lambda word: word.score)  # 最終的なスコアでソート
